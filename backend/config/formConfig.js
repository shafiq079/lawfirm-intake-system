...
export const getNextQuestion = (currentSectionId, currentQuestionIndex, formData) => {
  const section = formConfig[currentSectionId];
  if (!section) return null;

  // Find the next unanswered question in the current section
  for (let i = currentQuestionIndex; i < section.fields.length; i++) {
    const field = section.fields[i];
    // Check if the field is required and not yet filled
    if (field.validation?.required && !formData[currentSectionId]?.[field.id]) {
      return { sectionId: currentSectionId, fieldId: field.id, question: field.question, questionIndex: i };
    }
  }

  // If all required fields in the current section are answered, move to the next section
  const sectionIds = Object.keys(formConfig);
  const currentSectionIndex = sectionIds.indexOf(currentSectionId);

  for (let i = currentSectionIndex + 1; i < sectionIds.length; i++) {
    const nextSectionId = sectionIds[i];
    const nextSection = formConfig[nextSectionId];
    if (nextSection) {
      // Find the first required field in the next section
      for (let j = 0; j < nextSection.fields.length; j++) {
        const field = nextSection.fields[j];
        if (field.validation?.required && !formData[nextSectionId]?.[field.id]) {
          return { sectionId: nextSectionId, fieldId: field.id, question: field.question, questionIndex: j };
        }
      }
    }
  }
